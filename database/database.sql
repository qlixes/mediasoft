...
insert into lisparentsatuan(idsatuankoli, idsatuanbesar) values (1,1);
...
insert into listparentsatuandetail(idparentsatuan,idsatuankecil) values (1,1);
...
-- @LEVEL 
-- tiap level akan memiliki kaki sesuai dengan struktur organisasi
...
-- @LOGIN
create table listlogin(idlogin integer not null primary key autoincrement, userlogin varchar(64) not null unique, passlogin varchar(128) not null, idleveldetail int not null references listleveldetail(idleveldetail));
create table listlogindetail(idlogin integer not null primary key, namalogin varchar(64) not null,  idlokasi int not null references listlokasi(idlokasi));
